...
namespace BitSequenceTests {

void testConstructors() {
  BitSequence seq1;
  assertEqual(seq1.getLength(), 0, "BitSequence default constructor");

  BitSequence seq2("1010");
  assertEqual(seq2.getLength(), 4, "BitSequence from string");
  assertEqual(seq2.get(0).get(), true, "BitSequence string: first bit");
  assertEqual(seq2.get(3).get(), false, "BitSequence string: last bit");

  Bit bits[] = {Bit(true), Bit(false), Bit(true)};
  BitSequence seq3(bits, 3);
  assertEqual(seq3.getLength(), 3, "BitSequence from array");
  assertEqual(seq3.get(0).get(), true, "BitSequence array: first");
  assertEqual(seq3.get(2).get(), true, "BitSequence array: last");

  try {
    BitSequence seq4("102");
    printTestResult("BitSequence invalid char did not throw", false);
    ++testsFailed;
  } catch (const std::invalid_argument &) {
    printTestResult("BitSequence invalid char throws", true);
    ++testsPassed;
  }
}
...
void testAppendPrependInsertRemove() {
  BitSequence seq("01");
  Sequence<Bit> *newSeq = seq.append(Bit(true));
  assertEqual(newSeq->getLength(), 3, "append length");
  assertEqual(newSeq->get(2).get(), true, "append value");
  delete newSeq;

  newSeq = seq.prepend(Bit(false));
  assertEqual(newSeq->getLength(), 3, "prepend length");
  assertEqual(newSeq->get(0).get(), false, "prepend value");
  delete newSeq;

  newSeq = seq.insertAt(Bit(true), 1);
  assertEqual(newSeq->getLength(), 3, "insertAt length");
  assertEqual(newSeq->get(1).get(), true, "insertAt value");
  delete newSeq;

  newSeq = seq.remove(0);
  assertEqual(newSeq->getLength(), 1, "remove length");
  assertEqual(newSeq->get(0).get(), true, "remove result (second bit)");
  delete newSeq;

  assertEqual(seq.getLength(), 2, "original unchanged after operations");
}

void testGetSubsequenceConcat() {
  BitSequence seq("1010");
  Sequence<Bit> *sub = seq.getSubsequence(1, 2);
  assertEqual(sub->getLength(), 2, "getSubsequence length");
  assertEqual(sub->get(0).get(), false, "sub first");
  assertEqual(sub->get(1).get(), true, "sub second");
  delete sub;

  BitSequence other("11");
  Sequence<Bit> *concat = seq.concat(other);
  assertEqual(concat->getLength(), 6, "concat length");
  assertEqual(concat->get(4).get(), true, "concat first of other");
  assertEqual(concat->get(5).get(), true, "concat second of other");
  delete concat;
}
...
void testBitwiseOperations() {
  ...
  assertEqual(andRes->getLength(), 4, "And length");
  assertEqual(andRes->get(0).get(), true, "And 1&1");
  assertEqual(andRes->get(1).get(), false, "And 0&1");
  assertEqual(andRes->get(2).get(), false, "And 1&0");
  assertEqual(andRes->get(3).get(), false, "And 0&0");
  delete andRes;

  BitSequence *orRes = a.Or(b);
  assertEqual(orRes->get(0).get(), true, "Or 1|1");
  assertEqual(orRes->get(1).get(), true, "Or 0|1");
  assertEqual(orRes->get(2).get(), true, "Or 1|0");
  assertEqual(orRes->get(3).get(), false, "Or 0|0");
  delete orRes;

  BitSequence *xorRes = a.Xor(b);
  assertEqual(xorRes->get(0).get(), false, "Xor 1^1");
  assertEqual(xorRes->get(1).get(), true, "Xor 0^1");
  assertEqual(xorRes->get(2).get(), true, "Xor 1^0");
  assertEqual(xorRes->get(3).get(), false, "Xor 0^0");
  delete xorRes;

  BitSequence *notRes = a.Not();
  assertEqual(notRes->getLength(), 4, "Not length");
  assertEqual(notRes->get(0).get(), false, "Not 1");
  assertEqual(notRes->get(1).get(), true, "Not 0");
  delete notRes;

  try {
    BitSequence c("1");
    a.And(c);
    printTestResult("And with different lengths did not throw", false);
    ++testsFailed;
  } catch (const std::invalid_argument &) {
    printTestResult("And with different lengths throws", true);
    ++testsPassed;
  }
}
...
void testMap() {
  ...
  Sequence<int> *mapped = Map(seq, square);
  assertEqual(mapped->GetCount(), 4, "Map count");
  assertEqual(mapped->Get(0), 1, "Map 1^2");
  assertEqual(mapped->Get(1), 4, "Map 2^2");
  assertEqual(mapped->Get(2), 9, "Map 3^2");
  assertEqual(mapped->Get(3), 16, "Map 4^2");
  delete mapped;
}
...
void testReduce() {
  ...
  assertEqual(result, 10, "Reduce sum");
}

void testWhere() {
  int arr[] = {1, 2, 3, 4, 5, 6};
  MutableArraySequence<int> seq(arr, 6);
  Sequence<int> *filtered = Where(seq, isEven);
  assertEqual(filtered->GetCount(), 3, "Where count");
  assertEqual(filtered->Get(0), 2, "Where first even");
  assertEqual(filtered->Get(1), 4, "Where second even");
  assertEqual(filtered->Get(2), 6, "Where third even");
  delete filtered;
}
} // namespace MapReduceTests
...
void runAllTests() {
  ...
  std::cout << "\nBitSequence:\n" << std::endl;
  {
    using namespace BitSequenceTests;
    testConstructors();
    testAppendPrependInsertRemove();
    testGetSubsequenceConcat();
    testBitwiseOperations();
  }

  std::cout << "\nMapReduce:\n" << std::endl;
  {
    using namespace MapReduceTests;
    testMap();
    testReduce();
    testWhere();
  }
