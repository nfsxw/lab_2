...
class ArraySequence : public Sequence<T> {
	...
	const T& getFirst() const override {
		if (data.getSize() == 0) throw std::out_of_range("ArraySequence->getFirst: sequence is empty");
		return data.get(0);
	}

	const T& getLast() const override {
        	if (data.getSize() == 0) throw std::out_of_range("ArraySequence->getLast: Array is empty");
	...
	Sequence<T>* insertAt(const T& value, int index) override {
        	if (index < 0 || index > data.getSize()) throw std::out_of_range("ArraySequence->insertAt: index out of range");

		if (index == 0) return prepend(value);
		if(index == data.getSize()) return append(value);
	...
	Sequence<T>* remove(int index) override {
        	if (index < 0 || index >= data.getSize()) throw std::out_of_range("ArraySequence->remove: index out of range");
	...
	Sequence<T>* getSubsequence(int start, int end) const override {
        	if (start < 0 || end >= data.getSize() || start > end) throw std::out_of_range("ArraySequence->getSubsequence: invalid indices");
